...
Siga as link:kvm-host-configuration-and-preparation-checklist.html["lista de verificação de preparação do anfitrião"] e revise as informações necessárias para o link:reference_chk_deploy_req_info.html["instalação do utilitário de implantação"] e o link:reference_chk_select_req_info.html["Instalação do ONTAP Select"] para mais informações.
